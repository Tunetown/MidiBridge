...
(function(root, mod) {
  ...
  function readToken_mult_modulo(code) { // '*%'
    ...
    // *a, b = list
    // FIXME Python3 only!
    if (!tokType || tokType === _comma) {
      return finishOp(_star, 1);
    }
  ...
  function readWord1() {
    ...
    for (;;) {
      ...
      if (isIdentifierChar(ch)) {
        ...
        containsEsc = true;
        var nextChar = input.charCodeAt(++tokPos);

        // if at end of line, consider as end of word; and skip the line breaks
        if (isNewline(nextChar)) {
          ++tokPos;
          if (isNewline(input.charCodeAt(tokPos))) ++tokPos;  // \r\n
          return word;
        }
  ...
  function unexpected(tok) {
    raise(tokStart, `Unexpected token ${JSON.stringify(tok || tokType || tokVal)}`);
  }

  // Verify that a node is an lval - something that can be assigned
  // to.

  var allowedLValTypes = ["Identifier", "MemberExpression", "StarExpression"];
  function checkLVal(expr) {
    if (!allowedLValTypes.includes(expr.type))
  ...
  function parseStatement() {
    ...
    case _nonlocal:
      ...
      return finishNode(node, "NonLocalStatement");
    ...
    case _del:
      next();
      node.deleted = parseExpression();
      return finishNode(node, "DelStatement");
    ...
    case _yield:
      next();
      node.argument = (tokType ===_newline || tokType === _eof) ? null : parseExpression();
      return finishNode(node, "YieldStatement");
    ...
    case _try:
      ...
      if (eat(_else)) {
        expect(_colon);
        node.else = parseSuite();
      }

      if (eat(_finally)) {
        expect(_colon);
        node.finalizer = parseSuite();
      }
    ...
    case _assert:
      ...
      if (eat(_comma)) node.msg = parseExprAtom();
      return finishNode(node, "AssertStatement");
    ...
    default:
      ...
      //FIXME bad format (consequent has full expression, alternate only has value)
      if (tokType === _if) {
        node.consequent = expr;
        next();
        if (tokType === _parenL) node.test = parseParenExpression();
        else node.test = parseExpression();
        expect(_else);
        node.alternate = parseExpression();
        return finishNode(node, "InlineIfStatement");
      }
  ...
  function parseImport(node) {
    node.import = tokVal === "*" && eat(_multiplyModulo) ? "*" : parseExpression();
    if (eat(_as)) node.as = parseIdent();
  ...
  function parseMaybeAssign(noIn) {
    ...
    if (tokType.isAssign) {
      ...
      var opName = _assignOps[node.operator];
      if (opName) {
        var right = nc.createNodeSpan(node.right, node.right, "CallExpression");
        right.callee = nc.createNodeOpsCallee(right, opName);
  ...
  function parseExprAtom() {
    ...
    case _lambda:
      ...
      return finishNode(node, "LambdaStatement");
    ...
    case _star: // *a, b = list
      ...
      var node = startNode();
      node.var = parseIdent();
      return finishNode(node, "StarExpression");

    case _at:
      next();
      var node = startNode();
      node.annotation = parseExpression();
      return finishNode(node, "AnnotationExpression");
  ...
  var validPropertyNameTypes = [_num, _string, _true, _false];
  function parsePropertyName() {
    if (validPropertyNameTypes.includes(tokType)) return parseExprAtom();
  ...
  function parseIdent(liberal) {
    ...
    var lastTokEnd = tokEnd;
    next();

    // f"str"
    //FIXME Python3 only!
    //FIXME move out of parseIdent if possible
    if (node.name === "f" && tokStart === lastTokEnd && tokType === _string) {
      node.format = tokVal;
      next();
      return finishNode(node, "FormatStatement");
    }
